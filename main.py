...
def register_email_valid():
    ...
    if user:
        ...
        # 세션에 인증번호와 생성 시간 저장
        session[f'otp_{email}'] = random_auth_number
        session[f'time_{email}'] = current_timestamp
...
def register_auth_number_valid():
    ...
    session_otp = session.get(f'otp_{email}')  # 세션에 저장된 인증번호 가져오기
    session_time = session.get(f'time_{email}')  # 세션에 저장된 생성 시간 가져오기
    print(otp, email)
    print(session_otp, session_time)
    if otp == session_otp:
        return jsonify(result=session_otp)
    else:
        return "No Auth"
